function s = myste(x,flag,dim)
%MYSTD Robust standard deviation
%
%        S = MYSTE(X,FLAG,DIM)
%
% Compute the standard error of matrix X, along dimension DIM,
...
if flag
	s = sum(x2,dim) ./ (sum(1-nans,dim).^2);
else
	s = sum(x2,dim) ./ ((sum(1-nans,dim)-1).*sum(1-nans,dim));
